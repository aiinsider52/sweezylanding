...
const DEPTH_COPY: Record<Locale, { guide: string; choose: string; bestFor: string; route: string; watch: string; checklist: string; sources: string; planning: string; planningBody: string; planningCta: string; cantonCta: string }> = {
  en: { guide: "In-depth guide", choose: "Choose your version of the day", bestFor: "Best for", route: "Plan", watch: "Check first", checklist: "Before leaving", sources: "Sources and review", planning: "Plan the whole trip", planningBody: "Transport, weather, maps and a backup route — one practical checklist for travelling around Switzerland.", planningCta: "Open travel planner", cantonCta: "Open canton guide" },
  ...
  de: { guide: "Ausführlicher Guide", choose: "Passenden Tagesplan wählen", bestFor: "Geeignet für", route: "Plan", watch: "Zuerst prüfen", checklist: "Vor der Abfahrt", sources: "Quellen und Prüfung", planning: "Ganze Reise planen", planningBody: "Verkehr, Wetter, Karten und Ersatzroute — eine praktische Checkliste für Reisen durch die Schweiz.", planningCta: "Reiseplaner öffnen", cantonCta: "Kantonsguide öffnen" },
};
...
export default function DestinationPage({ params }: { params: { locale: string; slug: string } }) {
  ...
  return <main lang={locale} className={styles.page}>
    ...
    <article className={styles.shell}>
      ...
      <aside className={styles.planningBridge}><div><p className={styles.eyebrow}>{depthCopy.planning}</p><h2>{depthCopy.planning}</h2><p>{depthCopy.planningBody}</p></div><div className={styles.planningActions}><Link href={`/${locale}/planning`}>{depthCopy.planningCta} ↗</Link>{place.cantonGuideSlug ? <Link href={`/${locale}/guides/${place.cantonGuideSlug}`}>{depthCopy.cantonCta} ↗</Link> : null}</div></aside>

      <section className={styles.source} aria-labelledby="destination-sources-title"><p id="destination-sources-title">{depth ? `${depthCopy.sources}: ${depth.reviewedAt}.` : copy.source}</p><a href={place.sourceUrl} target="_blank" rel="noreferrer noopener">Switzerland Tourism ↗</a>{additionalSources.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer noopener">{source.name} ↗</a>)}{credits.length ? <span className={styles.credits}>Images: {credits.map((credit, index) => <a key={credit.pageUrl} href={credit.pageUrl} target="_blank" rel="noreferrer noopener">{credit.creator} · {credit.license}{index < credits.length - 1 ? ", " : ""}</a>)}</span> : null}</section>
