const clusters = {
  ...
  permits_registration: { count: 20, topics: ["реєстрація в Gemeinde", "дозвіл на проживання", "строки реєстрації", "потрібні документи", "міграційний офіс"], url: "/uk/guides/zurich" },
...
const prompts = Object.entries(clusters).flatMap(([cluster, config]) => Array.from({ length: config.count }, (_, index) => ({
  ...
  cluster,
  prompt: frames[index % frames.length](config.topics[index % config.topics.length]),
...
const header = "id,cluster,prompt,expected_url,model,date,cited,cited_url,position,notes";
const rows = prompts.map((item) => [item.id,item.cluster,item.prompt,item.expected_url,"","","","","",""] .map((value)=>`"${String(value).replaceAll('"','""')}"`).join(","));
if (process.argv.includes("--csv")) process.stdout.write(`${header}\n${rows.join("\n")}\n`);
else console.log(`AI citation benchmark valid: ${prompts.length} prompts across ${Object.keys(clusters).length} clusters.`);
